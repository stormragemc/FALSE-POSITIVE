...
runtime (`Sidecar/tts.py`), not from a file here — these Spassky clips are
only the two pre-rendered cutscene lines (P1's answer and the placeholder
ending line). `Sidecar/config.py`'s `ELEVENLABS_VOICE_ID` still needs to be
pointed at a male voice on the hosted account to match; that is a deployment
change outside this repo (see `docs/GAME_COMPLETION_PLAN.md` §7, B0).
