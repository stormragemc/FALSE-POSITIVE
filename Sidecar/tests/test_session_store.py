...
class InMemorySessionStoreTests(unittest.TestCase):
    def test_idle_sessions_expire_after_the_configured_ttl(self):
        now = [100.0]
        ...
        store.commit("stale", [{"role": "user", "content": "private testimony"}])

        now[0] = 111.0

        self.assertEqual(store.expire_idle(), ["stale"])
        self.assertEqual(store.history("stale"), [])
