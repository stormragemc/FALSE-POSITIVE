...
## 3. Create `MainMenu.unity`

`Assets/_Project/Scenes/MainMenu.unity`.

```
MainMenu
├── Main Camera + a Light   (this scene owns its own camera/listener — fine,
│                              since Interrogation/memory scenes are inactive
│                              while this one is active)
├── Environment              <- cabin exterior at night, reuse
│                                CabinNight/Data/Snowflake.png +
│                                SnowParticle material for falling snow;
│                                a still shot or very slow camera drift
├── Canvas
│   └── Play / Settings / Quit buttons
└── MainMenuController.cs    <- wire playButton/settingsButton/quitButton
                                 (Settings opens GameFlowDirector.SettingsPanel,
                                 which lives in _Persistent — see step 1 —
                                 so there is no settingsPanel field to wire here)
```

Storm ambience: a looping `AudioSource` on `MainMenu`'s root, wind/snow bed.
